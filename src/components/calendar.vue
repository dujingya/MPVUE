        ...
        <block v-for="(object, index) in dateList" :key="index">
            ...
            <view class="days">
                ...
                <view class="grid white-color" :class="[{pinkBg:item.status==3}]" v-for="(item, n) in object.days" :key="n"  @click="selectAction(object,item)">
                    <view :class="[{disable:(item.status==0||!item.data),enable:!(item.status==0||!item.data),borderRadius:item.status==3,day: true,lastWeek: item.weekIndex==6,lastWeek: item.weekIndex==0}]">
                        {{item.day}}
                    </view>
    ...
    export default {
        ...
        methods: {
            ...
            selectAction(e, d) {
                ...
                let warnMsg = ''
                if (appoint) {
                    if (parseInt(appoint.rest) === 0) {
                        warnMsg = '已经预约满,请选择其他日期'
                        this.toShowModal(warnMsg)
                        return
                    }
                } else {
                    warnMsg = '当前日期不可预约,请选择其他日期'
                    console.log(warnMsg)
                    this.toShowModal(warnMsg)
                    return
                }
